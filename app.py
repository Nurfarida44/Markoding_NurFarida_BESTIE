...
# Mencari file harga di folder utama repositori GitHub
...
try:
    if df_harga is not None:
        ...
        if st.button("Hitung Rekomendasi Menu"):
            # Membaca file data bahan pangan yang ada di GitHub
            ...
            def deteksi_alergen(nama_bahan):
                ...
                if 'telur' in nama_lower:
                    return 'Telur'
                elif any(kata in nama_lower for kata in ['ikan', 'teri', 'udang', 'mujair', 'seafood', 'kepiting', 'cumi']):
                    return 'Ikan/Seafood'
                elif any(kata in nama_lower for kata in ['kacang', 'tempe', 'tahu', 'kedelai']):
                    return 'Kacang/Kedelai'
                elif 'susu' in nama_lower:
                    return 'Susu'
                else:
                    return 'Aman (Non-Alergen)'
            ...
            for index, row in df_harga_lokal.iterrows():
                ...
                if 'telur' in komoditas_nama:
                    kamus_harga_live['Telur Ayam'] = harga_nilai / 10
                elif 'beras' in komoditas_nama:
                    kamus_harga_live['Nasi Putih'] = (harga_nilai / 10) * 0.4
