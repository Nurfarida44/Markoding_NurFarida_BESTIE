...
if files_harga:
    ...
    st.error(f"Waduh! Python sudah mencari langsung ke folder '{folder_target}', tapi tidak ada file yang mengandung kata 'harga'.")
...
try:
    ...
    if st.button("Hitung Rekomendasi Menu"):
        df_gizi = pd.read_csv("D:/BESTIE/data_bahan_pangan.csv", sep=None, engine='python')
        ...
        if df_rekomendasi.empty:
            st.warning(f"Mohon maaf, tidak ditemukan bahan makanan aman.")
        else:
            ...
            with st.spinner("Asisten BESTIE sedang menganalisis kandungan gizi lewat API Gemini..."):
                try:
                    # Kunci API Resmi Berformat Baru Anda
                    ...
                    # PAYLOAD YANG SUDAH DITUTUP SEMPURNA DENGAN KURUNG KURAWAL
                    payload = {
                        "contents": [{
                    ...
                    if 'candidates' in hasil_json and len(hasil_json['candidates']) > 0:
                        ...
                        # Jika ada pesan error dari Google, tampilkan secara rapi untuk debug
